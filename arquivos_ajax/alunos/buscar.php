<?php
	...
	if(empty($valor)){
		...
		$sql = 'SELECT * FROM alunos WHERE cod_alunos LIKE "%'.$valor.'%"';
		$query = $bd->select($sql);
		$num_rows = $bd->num_rows($query);

		if($num_rows < 1){

			echo "<p class='notFound'>Nenhum resultado encontrado.</p>";
			...
			$sql = "SELECT * FROM alunos WHERE cod_alunos LIKE '%".$valor."%' LIMIT $inicio, $qnt_result_pg";
			$query = $bd->select($sql);
